...
const sampleInventory: Inventory[] = [
  { productId: '1', quantity: 25, warehouseId: 'warehouse-west', lastRestocked: Date.now() - 7200000 },
  { productId: '1', quantity: 30, warehouseId: 'warehouse-east', lastRestocked: Date.now() - 3600000 },
  { productId: '2', quantity: 150, warehouseId: 'warehouse-west', lastRestocked: Date.now() - 1800000 },
  { productId: '2', quantity: 120, warehouseId: 'warehouse-east', lastRestocked: Date.now() - 900000 },
  { productId: '3', quantity: 80, warehouseId: 'warehouse-west', lastRestocked: Date.now() - 14400000 },
  { productId: '3', quantity: 95, warehouseId: 'warehouse-east', lastRestocked: Date.now() - 10800000 }
];
...
export class ProductRepository {
  ...
  static getAll(category?: string, minPrice?: number, maxPrice?: number): Product[] {
  ...
  static update(id: string, productData: Partial<Omit<Product, 'id' | 'createdAt'>>): Product | null {
    const existing = products.get(id);
    if (!existing) return null;

    // Filter out id and createdAt from update data to prevent overwriting
    // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unused-vars
    const { id: _id, createdAt: _createdAt, ...safeUpdateData } = productData as any;
  ...
  static paginate(pageSize: number, pageToken: string, category?: string, minPrice?: number, maxPrice?: number): { products: Product[]; nextPageToken: string; totalCount: number } {
    ...
    const nextPageToken = endIndex < allProducts.length ? endIndex.toString() : '';
